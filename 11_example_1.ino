...
#define _DUTY_MIN 730 // servo full clockwise position (0 degree)
...
float _DUTY_NEU = (_DUTY_MIN+_DUTY_MAX)/2;
Servo myservo;
...
void loop() {
  ...
  if(dist_filtered<=180){
    ...
    _DUTY_NEU = _DUTY_MIN+(_DUTY_MAX-_DUTY_MIN)/180*(dist_filtered-180);
    myservo.writeMicroseconds(_DUTY_NEU);
  }
